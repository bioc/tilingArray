...
# seg-tot-x is total-cDNA, while Lior's total-RH6 seems to refer to the total-RNA-RH6 samples
...
myPlot = function(x, y, fac, txtLabel,mypch=16, ...){
  ...
  spearmanCor <- round(cor(x, y, method="spearman"),digits=2)
  text(max(x),min(y),labels=paste("Spearman Correlation =",spearmanCor), pos=2)
...
myPlot(rtpcrRes$array1, -rtpcrRes$Mean.dT, gene, region,mypch=mypch, xlab="Array Expression (polyA)", ylab="rtPCR (polyA) [-Ct]")
