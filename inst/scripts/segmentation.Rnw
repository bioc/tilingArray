...
seg = cache("seg", {
  ...
  seg = segment(y, x=sprb$mid, maxseg=maxseg, maxk=3000)
})
@ 
...
source("/homes/huber/madman/Rpacks/tilingArray/R/methods-segmentation.R")
plot(segc, maxseg, pch=".", xlim=c(0, 40000))
@ 
\myincfig{segmentation-plotseg}{\textwidth}{% 
Segmentation with confidence intervals.
}

<<plotAlongChrom, fig=TRUE, include=FALSE, width=8, height=7>>=
data("gff")
source("/homes/huber/madman/Rpacks/tilingArray/R/plotAlongChrom.R")
plotAlongChrom(segc, chr=1, coord = 1000*c(76.2, 92.8),
               gff = gff)
