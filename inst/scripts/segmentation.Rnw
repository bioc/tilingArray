...
into the slot \code{x}, the $x$-coordinates of the probes, 
and into the slot \code{flag}, the uniqueness status of the probes 
(0 iff the probe has exactly one match in the genome).

Having to access the \code{x} and \code{flag} slots directly, in the
...
This is simply a call the \Rfunction{confint} method of the 
...
of the \Rclass{segmentation} class.
%
...
Figure~\ref{segmentation-alongChrom}. We run the same computations as
...
data("gff")
## source("/homes/huber/madman/Rpacks/tilingArray/R/plotAlongChrom.R")
segObj = new.env(parent = baseenv())
assign("1.+", segwi, segObj)
assign("1.-", segci, segObj)
plotAlongChrom(segObj, nrSegments=nseg, chr=1, coord = 1000*c(76.2, 92.8),
               gff = gff)
@ 
%
\myincfig{segmentation-alongChrom}{\textwidth}{% 
