...
This script demonstrates how to run \Rpackage{tilingArray}'s
main segmentation function on the \Rpackage{davidTiling} data. 
...
<<convertDavidTiling, echo=FALSE, results=hide>>=
options(length=60)
davidTiling <- as(davidTiling,"ExpressionSet")
@ 
...
\textit{Assessing signal/noise ratio before and after normalization}
 (\texttt{assessNorm.Rnw}). 
...
not dominating the segmentation algorithm,
...
<<avoidOversampling>>=
chr1p.probeStarts <- sort(get("1.+.start",env=probeAnno))
...
\myincfig{segmentation-histprobs}{0.8\textwidth}{% 
(a):
...
\Robject{nrBasesPerSegment}, a lower bound for the expected
length of individual segments,
and \Robject{maxk}, the maximum length of individual segments. 
From the supplied \Robject{nrBasesPerSegment}, 
the maximum number of segments that the algorithm is going to consider,
\Robject{maxseg}, are computed by dividing the length of the
chromosome by \Robject{nrBasesPerSegment}.
...
\times 3,000 = 22,500$ bases. Note that there setting the lower
bound for the \emph{average} segment length, does not enforce
a minimum length restriction for individual segments.

For demonstration purpose, we first perform the segmentation
for the Watson strand of chromosome 1 solely.
%
<<useSegChrom, eval=TRUE, results=hide>>=
segEnv <- segChrom(xn[,xn$nucleicAcid=="poly(A) RNA"], 
                   probeAnno=probeAnno, chr="1", strands="+",
                   nrBasesPerSegment = 750)
@ 

The resulting object is environment holding the segmentation
results for each chromosome and strand as individual
objects of \Rclass{segmentation}.
...
For individual segmentations, we can compute confidence
intervals for each segment border.
...
tilingArray:::plotPenLL(segChr1p, extrabar=c("black"=round(segChr1p@x[length(segChr1p@x)]/1500)), type="l", lwd=2) 
@ 
%\myincfig{segmentation-modelSelect}{0.6\textwidth}{% 
...
%--------------------------------------------------
<<confintwidthscalc>>=
segMultiCI = confint(segChr1p, parm=as.integer(c(112, 153, 194, 235, 276)),
  level=confintLevel) 
...
\myincfig{segmentation-confintwidthsecdf}{0.6\textwidth}{% 
Size of the confidence intervals as a function of $S$.
Cumulative distribution functions (CDFs) for the distributions of confidence 
interval widths for $S=$\Sexpr{paste(nBp, collapse=", ")}. 
For larger $S$, the confidence intervals are wider.}
...
\Robject{for}--loop below could easily be parallelized if needed
since the computations for different chromosome strands are 
independent of each other. 
%
<<computeSegmentations, eval=FALSE>>=
segEnv <- segChrom(xn[,xn$nucleicAcid=="poly(A) RNA"], 
                   probeAnno=probeAnno, chr=c(1,2,5,9,13,14,15), 
                   strands=c("+","-"), nrBasesPerSegment = 1500)
@ 
<<computeSegmentationsDo, echo=FALSE, results=hide>>=
cache(segEnv <- segChrom(xn[,xn$nucleicAcid=="poly(A) RNA"], 
                         probeAnno=probeAnno, chr=c(1,2,5,9,13,14,15), 
                         strands=c("+","-"), nrBasesPerSegment = 1500))
@ 
%
\Robject{segEnv} is an environment holding the 14 
individual segmentation results.
%

\section{Visualizing segmentations with \Rfunction{plotAlongChrom}} 
...
@ 
%
The folowing source code was used to generate Figure 2 of~\cite{David2006}.
%
