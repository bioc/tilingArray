...
Extract for all probes that map to chromosome 1, "+" strand their
...
In same cases, the spacing is now wider, probably due to updates in the 
genome assembly or sequence between when the chip was designed and 
...
seg = cache("seg", {
  ...
  seg = segment(y, maxseg=maxseg, maxk=20000)
})
@ 
