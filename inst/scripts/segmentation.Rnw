...
\end{eqnarray}
%
We define a function \Rfunction{penLL} that implements these
%
<<defpenLL>>
...
@
% 
and plot them as functions of $S$, see Figure~\ref{segmentation-modelSelect
%
<<modelSelect, fig=TRUE, eps=FALSE, include=FALSE, width=4.5, height=4.5>>=
...
@ 
%
\myincfig{segmentation-modelSelect}{\textwidth}{% 
...
$S$. Vertical green bar corresponds to optimal $\log L_{\mbs{BIC}}$,
vertical grey bar to our ``subjective'' choice of average segment
length 1,500 bases.}
...
``-'' strand of chromosome 1.  For Figure~\ref{segmentation-fig2}, we
also call it on a number of other chromosomes. This computation will
take a couple of hours. Note that it can be trivially parallelized
since the computations for different chromosome strands are
independent of each other. Finally, we collect all results in the
environment \Robject{segObj}.
%
<<makeSegObj>>=
toDo = c("1.-", "2.+", "2.-", "5.+", "5.-", "9.+", "9.-", "13.+", "13.-", "14.+", "14.-","15.+", "15.-")
for(w in toDo) {
  fn = paste(w, "rda", sep=".")
  if(!file.exists(fn)) {
...
Along-chromosome plot similar to Figure 1 in the paper~\cite{David2006}.}
\myincfig{segmentation-fig2}{\textwidth}{% 
Along-chromosome plots similar to Figure 2 in the paper~\cite{David2006}.}
%
...
<<fig2, fig=TRUE, eps=FALSE, include=FALSE, width=11, height=5.5>>=
dx = 0.20
dy = 0.05
