...
%--------------------------------------------------
\subsection{Can the size of the confidence intervals 
...
## br = seq(0, maxx, by=2)
## mapply(function(x, y) hist(x[x<=maxx], breaks=br, main=y, col="orange"),
##   confintwidths, nBp)
...
Can the size of the confidence intervals 
be used for model selection?
Cumulative distribution functions (CDFs) for the distributions of confidence 
interval widths for $S=$\Sexpr{paste(nBp, collapse=", ")}. For larger $S$, the confidence 
intervals also become wider. 
}
...
%
<<fig2-bioinf, fig=TRUE, eps=FALSE, include=FALSE, width=11, height=5.5>>=
dx = 0.10
grid.newpage()
pushViewport(viewport(x=0.01, width=0.98, height=0.97, just=c("left", "center"),
                      layout=grid.layout(1, 7,
                        height=c(1),
                        width =c(1, dx, 1, dx, 1, dx, 1))))
         
## 13:550k splicing RPS16A, RPL13B
myPlot(1, 1, chr=13, coord = c(550044, 553360), main="a")
## MET7, novel architecture
myPlot(1, 3, chr=15, coord = c(784700, 790000), main="b")
## 9:221-227: novel antisense SPO22
myPlot(1, 5, chr=9, coord = c(221000, 226500), main="c")
