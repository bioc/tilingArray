...
This vignetter demonstrates how to run \Rpackage{tilingArray}'s
...
take a couple of hours (about 18h on mine). Note that it 
could easily be parallelized if needed,
