...
Figure~\ref{segmentation-fig1}. To do this, we define the
function \Rfunction{segFun}. It encapsulates the complete set of
computations, as shown above, for one chromosome strand.  Its result
is a \Rclass{segmentation} object with confidence intervals.
%
<<segFun, results=hide>>=
segFun = function(chrstrd) {
...
for(w in toDo) {
  if(!file.exists(paste(w, "rda", sep="."))) {
    assign(w, segFun(w))
    save(list=w, file=paste(w, "rda", sep="."))
  }
}
