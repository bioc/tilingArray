  ...
  chrs = 1:17, verbose = TRUE)
{
  ...
  for(chr in chrs) { ## chrs (chromosomes) not defined anywhere!
    for(strand in c("+", "-")) {
      ...
      # WHY EXCLUDE REGIONS?
      allsel = (gff[, "chr"]==chr & !(gff[, "feature"] %in% c("chromosome"))) # 2005-08-25
      all.gff = gff[allsel, ]
      
      #return(same.gff)  # for debugging
      ...
      for(j in 1:cp) {
        ...
        stopifnot(all(nm3 %in% nm5)) # commented for debugging
        #isAll <- all(nm3 %in% nm5) # for debugging
        #if (!isAll) {cat("Error A");return(list(nm3, nm5))} # for debugging
