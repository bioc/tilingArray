...
scratch. Eventually they will go away, and you can safely ignore
them.
...
%
\myincfig{assessNorm-alongChromPlot}{0.8\textwidth}{% Scatterplot of
...
Noise $N$ is calculated as the weighted average of the differences
between 95\% and 5\% quantiles of the data within each of the control
regions,
\begin{equation}
N = \frac{1}{Q_{0.95}^{\mbs{Norm}}-Q_{0.05}^{\mbs{Norm}}}
\cdot\frac%
{\displaystyle\sum_{r\in\{\mbs{pos},\mbs{neg}\}}|r|\left(Q_{0.95}^r-Q_{0.05}^r\right)}
{\displaystyle\sum_{r\in\{\mbs{pos},\mbs{neg}\}}|r|}.
\end{equation}
Here, the symbol $r$ counts over the different regions, and $|r|$ is
the number of probes representing that region. The constant in the
denominator is the differences between 95\% and 5\% quantiles for the
standard Normal distribution, hence $N$ is equal to 1 if the data
comes from the standard Normal distribution.

Signal $S$ is calculated as the difference between the weighted averages of the means
of positive and negative control regions.
\begin{equation}
S =\frac{\displaystyle\sum_{r\in\{\mbs{pos}\}} |r| \mu_r}{\displaystyle\sum_{r\in\{\mbs{pos}\}}|r|}-
   \frac{\displaystyle\sum_{r\in\{\mbs{neg}\}} |r| \mu_r}{\displaystyle\sum_{r\in\{\mbs{neg}\}}|r|}.
\end{equation}

I have explored many variations of this calculation, using different
definitions of $N$, $S$, and of the control regions. Qualitatively, the 
results were always the same as those shown in the following.
%
...
sn = sapply(dat[2:6], withinAndBetween)
