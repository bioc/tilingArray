...
@ 
%
<<caching, results=hide, echo=FALSE>>=
...
dat[[1]] = log2(exprs(davidTiling)[ind, which(isRNA)[1]])
dat[[2]] = xMM[ind, 1]
dat[[3]] = log2(exprs(davidTiling)[ind, which(isDNA)[1]])
dat[[4]] = dat[[2]]-dat[[1]]
dat[[5]] = exprs(xnAllfeatures)[ind, 1]
dat[[6]] = exprs(xn)[ind, 1]
...
source("/home/huber/madman/Rpacks/tilingArray/R/comparisonPlot.R")
comparisonPlot(x, lapply(dat, "[", sel), yscale=ysc)
