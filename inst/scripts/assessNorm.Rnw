%\VignetteIndexEntry{Normalization overview and assessment of signal/noise ratio.}
...
%------------------------------------------------------------
\title{Normalisation with the \Rfunction{normalizeByReference} function
in the \Rpackage{tilingArray} packageAssessing signal/noise ratio before and after normalization}
%------------------------------------------------------------
...
The purpose of this vignette is to demonstrate the 
probe-response normalization provided by the function
\Rfunction{normalizeByReference} in the \Rpackage{tilingArray}
package, and to assess its performance.
We use the example data from our paper \cite{David2006};
the data are provided in the \Rpackage{davidTiling} package.
%
<<load, results=hide>>=
library("tilingArray")
library("vsn")
...
@
%
Note that some of the computations in this vignette will take a long time and
require Gigabytes of RAM. 

%--------------------------------------------------
\section{Selection of \emph{perfect match} and \emph{background} features}
%---------------------------------------------------
...
ispair = (!is.na(seqPM) & !is.na(seqMM) & 
    (seqPM==complementSeq(seqMM, start=13, stop=13)))
...
\Robject{seqPM=Scerevisiaetilingprobe\$sequence[ipm]} in
Section~\ref{sec:bgPM}.

<<GCBeforeAfter, fig=TRUE, width=7, height=7>>=
par(mfrow=c(1,2))
