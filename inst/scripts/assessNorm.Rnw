...
@ 
%
Noise $\sigma$ is calculated as the weighted average of the
differences between 97.5\% and 2.5\% quantiles of the data within each
of the control regions. The range between the 97.5\% and 2.5\%
quantiles contains 95\% of the data, while 5\% is outside the range.
\begin{equation}
...
\end{equation}
Here, the symbol $r$ counts over the different regions, and $n_r$ is
...
standard Normal distribution, hence $\sigma$ is equal to 1 if the data
come from the standard Normal distribution.

Signal $\Delta\mu$ is calculated as the difference between the weighted averages of the means
