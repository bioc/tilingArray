...
if(!exists("davidTiling"))
  data("davidTiling")
rownames(pData(davidTiling)) =  sampleNames(davidTiling)
@ 
%
...
%----------------------------------------
\section{Assessment}
For comparison, we also obtain the data if we do not throw out the weakest features.
<<normalizeWithoutThrowout>>=
xnAllfeatures = normalizeByReference(davidTiling[,isRNA], davidTiling[,isDNA], 
...
dat[[3]] = dat[[2]]-dat[[1]]
dat[[4]] = exprs(xnAllfeatures)[ind, 1]
dat[[5]] = exprs(xn)[ind, 1]
@ 
%
We select a 10kB region to fit on a plot,
<<selectRegionForPlot>>=
...
par(mfrow=c(length(dat), 1), mai=c(0.4, 0.6, 0.4, 0.01))
