...
%I have sprinkled \Robject{cache()} and
...
xwmm = normalizeByReferenceWithMM(davidTiling[, isRNA], davidTiling[, isDNA], PMind, MMind)
@ 
...
<<alongChromPlot, fig=TRUE, include=FALSE, height=11, width=8.4, eps=TRUE, pdf=FALSE>>=
...
writeLines(sub(" 3.00", " 1.50", readLines("assessNorm-alongChromPlot.eps"), fixed=TRUE),
...
<<GCBeforeAfter, fig=TRUE, include=FALSE, width=7, height=7>>=
par(mfrow=c(1,2))
