...
rgnf = range(rms)
cols = colorRamp(brewer.pal(11, "Spectral"))((rms[isel]-rgnf[1])/(rgnf[2]-rgnf[1])) / 256
cols = rgb(cols[,1], cols[,2], cols[,3])

for(i in 1:ncol(ysel))
  plot(xsel, ysel[,i], pch=20, col=cols, xlab="coordinates (chr 4-)", ylab="signal",
