...
%--------------------------------------------------
The data processing tasks in the following typically require a lot of CPU
times (in the order of days) and memory (several Gigabytes). Many of tasks
will therefore usually be performed in scripts rather than in interactive
sessions. Compute-heavy scripts (that in some cases can be run in parallel)
alternate with visualization-oriented scripts for analysis of (intermediate)
results.
