...
package. We use a small subset data from a expression profiling paper \cite{Xu2009}; The data
only include the region from 35000bp to 50000bp in yeast chromosome one. expression profiling 
is done in YPE and YPD condition, 3 replicates each. Further information about the experimental design 
...
The following code generate Figure~\ref{plotAlongChrom-alongChromDot1},a dot plot that averaged across all the hybes.
<<alongChromDot1,fig=TRUE, eps=FALSE, include=FALSE, width=16, height=6,echo=FALSE>>=
grid.newpage()
plotAlongChrom(segnf,chr=1, coord=c(35000,50000),what="dots", gff=gffSub)
@
\myincfig{plotAlongChrom-alongChromDot1}{0.96\textwidth}{% 
Along-chromosome dot plot of the averaged value across all the hybes.}

We could also make seprate dot plots for different hybes by setting the parameter\textit{sepPlot} as TRUE. The 
following code generate Figure~\ref{plotAlongChrom-alongChromDot2} that plot the expression for the two 
condition seprately.
<<alongChromDot2,fig=TRUE, eps=FALSE, include=FALSE, width=16, height=8,echo=FALSE>>=
...
force to take the average. a better alternative of display multiple hybes is to use the heatmap.        
The following code generate Figure~\ref{plotAlongChrom-alongChromHeatmap1} that makes the heatmap plot.
...
Start with R 2.11.0, the \Rpackage{grid} pacakge introduced the raster array image function 
\Rfunction{grid.raster} which is a faster and efficient way of generating heatmap images.Now the  
\Rpackage{tilingArray} package uses the \Rfunction{grid.raster} function as default to make heatmap
images replacing the previous \Rfunction{grid.rect} function. The choose of the two drawing function
can be changed by the parameter \textit{makeRasterImage}. The following code generate 
...
The following code generate Figure~\ref{plotAlongChrom-alongChromHeatmap3} 
that makes the raster heatmap plot using a blue gradient.
