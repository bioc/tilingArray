plotPenLL = function(seg, extrabar=numeric(0), type="b", pch=16, lwd=1) {
  cols = brewer.pal(3, "Set1")
  lls = cbind(logLik(seg), logLik(seg, penalty="AIC"), logLik(seg, penalty="BIC"))

  ylim = c(min(apply(lls, 2, quantile, probs=0.2)), max(lls))
  matplot(lls, type=type, col=cols, ylab="(penalized) log likelihood",
          lty=1, lwd=lwd, pch=pch, xlab="S", ylim=ylim)
